import React, { useState, useMemo } from 'react'
import { useFetch } from '../hooks/useFetch'
import { NewsCard } from '../components/indexComponents'

export const News = ({ url }) => {
  const [search, setSearch] = useState('')
  const [page, setPage] = useState(1)
  const newsPerPage = 6

  const { result: data, loading, error } = useFetch(url)

  const news = data?.results || []

  // Filter based on search
  const filteredNews = useMemo(() => {
    return news?.filter(article =>
      article.title.toLowerCase().includes(search.toLowerCase()) ||
      article.description?.toLowerCase().includes(search.toLowerCase())
    ) ?? []
  }, [news, search])

  // Paginate the filtered news
  const paginatedNews = useMemo(() => {
    const start = (page - 1) * newsPerPage
    const end = start + newsPerPage
    return filteredNews.slice(start, end)
  }, [filteredNews, page])

  console.log(filteredNews)
  const totalPages = Math.ceil(filteredNews.length / newsPerPage)

  return (
    <main>
    <div className="min-h-screen bg-gray-100 dark:bg-zinc-900 py-10 px-4">
      <h1 className="text-3xl font-bold mb-8 text-center text-zinc-900 dark:text-white">
        Latest Crypto News
      </h1>

      <div className="flex justify-center mb-6">
        <input
          type="text"
          value={search}
          onChange={(e) => {
            setSearch(e.target.value)
            setPage(1)
          }}
          placeholder="Search news..."
          className="w-full max-w-lg px-4 py-2 rounded-lg border dark:bg-zinc-800 dark:text-white dark:border-zinc-700 focus:outline-none"
        />
      </div>

      {loading && <p className="text-center text-zinc-600 dark:text-zinc-300">Loading...</p>}
      {error && <p className="text-center text-red-500">Error loading news.</p>}

      <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
        {paginatedNews.map((article, index) => (
          <NewsCard key={index} article={article} />
        ))}
      </div>

      {filteredNews.length > 0 && (
        <div className="flex justify-center items-center mt-10 space-x-4">
          <button
            className="px-4 py-2 bg-zinc-800 text-white rounded hover:bg-zinc-700 disabled:opacity-50"
            onClick={() => setPage((prev) => Math.max(1, prev - 1))}
            disabled={page === 1}
          >
            Previous
          </button>

          <span className="text-zinc-800 dark:text-white font-semibold text-lg">
            Page {page} of {totalPages}
          </span>

          <button
            className="px-4 py-2 bg-zinc-800 text-white rounded hover:bg-zinc-700 disabled:opacity-50"
            onClick={() => setPage((prev) => Math.min(totalPages, prev + 1))}
            disabled={page === totalPages}
          >
            Next
          </button>
        </div>
      )}
    </div>
    </main>
  )
}
